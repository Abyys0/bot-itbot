...
from ia_system.ia_brain import ia_brain
import logging
import asyncio
import json
...
async def on_message(message: discord.Message):
    """Intercepta mensagens para IA e filtros do Satoru"""
    ...
    # Filtro Satoru em servidores
    if message.guild:
        if satoru_security.active:
            await satoru_security.handle_message(message)
        
        # IA responde quando mencionado ou em canal específico
        bot_mentioned = bot.user in message.mentions
        ia_channel_name = "💬│chat-ia"
        is_ia_channel = message.channel.name == ia_channel_name if hasattr(message.channel, 'name') else False
        
        if bot_mentioned or is_ia_channel:
            ...
            if content:
                async with message.channel.typing():
                    try:
                        ...
                        # Se houver dados de busca, adiciona informações extras
                        ...
                        error_embed = discord.Embed(
                            title="❌ Erro",
                            description="Desculpe, tive um problema ao processar sua mensagem. Tente novamente!",
                            color=COLORS["error"]
                        )
...
@bot.command(name="ia_ativar")
@commands.has_permissions(administrator=True)
async def ia_ativar(ctx):
    """Cria canal exclusivo para IA"""
    try:
        guild = ctx.guild
        
        # Verificar se já existe
        existing_channel = discord.utils.get(guild.channels, name="💬│chat-ia")
        if existing_channel:
            await ctx.send(
                embed=discord.Embed(
                    title="⚠️ Canal já existe",
                    description=f"O canal {existing_channel.mention} já está ativo!",
                    color=COLORS["warning"]
                ),
                delete_after=10
            )
        ...
        channel = await guild.create_text_channel(
            "💬│chat-ia",
            overwrites=overwrites,
            topic=f"🤖 Converse com o {ia_brain.personality['name']}! Faça perguntas, peça buscas na internet e muito mais!",
            reason="Ativação da IA conversacional"
        )
        
        # Mensagem de boas-vindas no canal
        welcome_embed = discord.Embed(
            title=f"🤖 {ia_brain.personality['name']} - Inteligência Artificial",
            description=f"""
            Olá! Sou o **{ia_brain.personality['name']}**, a IA deste servidor! 👋
            ...
            **💬 Como me usar:**
            Basta enviar mensagens aqui neste canal, sem precisar de comandos!
            
            **📝 Exemplos:**
            • "Olá, quem é você?"
            • "Pesquise sobre FiveM"
            • "O que é Discord?"
            • "Busque informações sobre programação"
            
            **🔧 Características:**
            • {", ".join(ia_brain.personality['traits'][:4])}
            • Especializado em: {", ".join(ia_brain.personality['knowledge_areas'][:3])}
        ...
        welcome_embed.set_thumbnail(url=bot.user.avatar.url if bot.user.avatar else None)
        welcome_embed.set_footer(text="💡 Também respondo quando me mencionam em outros canais!")
        ...
        await ctx.send(
            embed=discord.Embed(
                title="✅ IA Ativada!",
                description=f"Canal {channel.mention} criado com sucesso!\n\n🤖 O {ia_brain.personality['name']} está pronto para conversar!",
                color=COLORS["success"]
            ),
            delete_after=15
        )
        
        logger.info(f"🤖 Canal de IA criado por {ctx.author}")
        
    except Exception as e:
        logger.error(f"Erro ao criar canal de IA: {e}")
        await ctx.send(
            embed=discord.Embed(
                title="❌ Erro",
                description=f"Não foi possível criar o canal de IA: {str(e)}",
                color=COLORS["error"]
            ),
            delete_after=10
        )

@bot.command(name="ia_status")
async def ia_status(ctx):
    """Mostra estatísticas da IA"""
    try:
        total_users = len(ia_brain.memory)
        total_messages = sum(user_data.get("message_count", 0) for user_data in ia_brain.memory.values())
        
        embed = discord.Embed(
            title=f"📊 Status da IA - {ia_brain.personality['name']}",
            color=0x00ff88,
            timestamp=discord.utils.utcnow()
        )
        ...
        embed.add_field(
            name="👥 Usuários",
            value=f"{total_users} usuários já conversaram comigo",
            inline=True
        )
        
        embed.add_field(
            name="💬 Mensagens",
            value=f"{total_messages} mensagens processadas",
            inline=True
        )
        
        embed.add_field(
            name="🧠 Personalidade",
            value=f"Sou {', '.join(ia_brain.personality['traits'][:3])}",
            inline=False
        )
        
        embed.add_field(
            name="📚 Áreas de Conhecimento",
            value=", ".join(ia_brain.personality['knowledge_areas'][:5]),
            inline=False
        )
        
        embed.set_thumbnail(url=bot.user.avatar.url if bot.user.avatar else None)
        embed.set_footer(text="💡 Me mencione ou converse no canal #chat-ia!")
        
        await ctx.send(embed=embed)
        
    except Exception as e:
        logger.error(f"Erro ao mostrar status da IA: {e}")
        await ctx.send("❌ Erro ao carregar status da IA.", delete_after=5)
